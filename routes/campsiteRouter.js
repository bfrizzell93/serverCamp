...
campsiteRouter
  ...
  .get((req, res, next) => {
    Campsite.find()
      ...
      .catch((err) => next(err)); //pass err to error handler in express app
  })
  .post(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {
    Campsite.create(req.body) //express will automatically convert req data to json
      .then((campsite) => {
    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (
          ...
          err = new Error("You can not update a comment from another person");
          err.status = 403;
          return next(err);
        } else {
          err = new Error(`Comment ${req.params.commentId} not found`);
    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (
          ...
          err = new Error("You can not delete a comment from another person");
          err.status = 403;
          return next(err);
        } else {
          err = new Error(`Comment ${req.params.commentId} not found`);
